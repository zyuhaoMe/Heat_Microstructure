  ...
  else
  {
    ...
    while (true)
    {
      if (timeVal <= timeEndList[i])
      {
        ...
        // used for test
        //cout << "["<< laserPositionX << " ; " << laserPositionY << "]" << endl;
...
void MultiTrackLaserClass::calLaserInfoMultiLine(
     double &laserPositionX,
     double &laserPositionY,
     double &speed,
     int &lineSegmentCurrent,
     bool &laserIsOn,
     double timeVal
     )
{
  int i = 0; // number of current line segment (zero-based index)
  bool keepLooping = true; // current loop is end

  if (timeVal >= timeEndList.back())
  {
    // if the all the scanning has already been finished
    laserIsOn = false;
    lineSegmentCurrent = numLineTrack - 1;
  }
  else
  {
    // decide which interval the timeVal is in
